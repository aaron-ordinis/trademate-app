...
export default function AdminDashboard() {
  ...
  const refreshBadgesTicker = useState(0)[1];
  ...
  useEffect(() => {
    const t = setInterval(() => refreshBadgesTicker(x => x + 1), 5000);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const row = Array.isArray(res.data) ? res.data[0] : res.data || null;

      let o = {};
      if (row) {
        o = {
          new_users_1d: row.new_users_1d || 0,
          new_users_7d: row.new_users_7d || 0,
          new_users_30d: row.new_users_30d || 0,
  ...
  useEffect(() => {
    const ch = supabase
      ...
      .on(
        ...
        (payload) => {
          setNotif(prev => ({ count: (prev.count || 0) + 1 }));
          setLastNoteAt(new Date().toISOString());
          Alert.alert('Notification', payload.new?.title || 'New admin notification');
        }
      )
      .on(
        'postgres_changes',
        ...
        () => {
          setInbox(prev => ({ unread: (prev.unread || 0) + 1 }));
  ...
  const mrr = useMemo(() => (overview.play_revenue_cents_30d || 0) / 100, [overview]);
  ...
  const supportTicketCount = Array.isArray(inbox.cachedTickets)
    ? inbox.cachedTickets.length
    : inbox.unread || 0;
  ...
  return (
    <View style={styles.screen}>
      ...
      >
        ...
        <View style={styles.card}>
          ...
          <View style={styles.quickActionsGrid}>
            ...
            <QuickActionButton
              icon="inbox"
              title="Support Inbox"
              subtitle={supportTicketCount === 1 ? "1 ticket" : `${supportTicketCount} tickets`}
            ...
            />
            {/* ✅ Added Push Test button */}
            <QuickActionButton
              icon="bell"
              title="Push Test"
              subtitle="Self push helper"
        ...
        <View style={styles.card}>
          ...
          <View style={styles.metricsGrid}>
            <MetricCard icon="user-plus" title="New Users (1d)" value={fmtInt(overview.new_users_1d)} color={SUCCESS} />
            <MetricCard icon="users" title="New Users (7d)" value={fmtInt(overview.new_users_7d)} color={SUCCESS} />
            <MetricCard icon="user-check" title="Total Users" value={fmtInt(overview.users_total)} color={BRAND} />
          </View>
          <View style={styles.metricsGrid}>
            <MetricCard icon="user-plus" title="New Users (30d)" value={fmtInt(overview.new_users_30d)} color={SUCCESS} />
          </View>
        </View>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.metricsGrid}>
            <MetricCard icon="download" title="Installs (7d)" value={fmtInt(overview.play_installs_7d)} color={SUCCESS} />
            <MetricCard icon="trending-up" title="Total Installs" value={fmtInt(overview.play_installs_total)} color={BRAND} />
            <MetricCard icon="smartphone" title="Active Devices" value={fmtInt(overview.play_active_devices)} color={BRAND} />
          </View>
          <View style={styles.metricsGrid}>
            <MetricCard icon="dollar-sign" title="Revenue (30d)" value={`${overview.play_currency} ${Math.round(mrr).toLocaleString()}`} color={SUCCESS} />
            <MetricCard icon="alert-triangle" title="Crashes (7d)" value={fmtInt(overview.play_crash_count)} color={overview.play_crash_count > 0 ? DANGER : MUTED} />
            <MetricCard icon="clock" title="Last Sync" value={lastSync} color={MUTED} small />
          </View>
        </View>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.departmentGrid}>
            <DepartmentCard icon="volume-2" title="Marketing" subtitle="Social media, campaigns & analytics" onPress={() => router.push('/(admin)/marketing')} />
            <DepartmentCard icon="headphones" title="Support" subtitle="Customer tickets & FAQ management" onPress={() =>
              router.push({
                pathname: '/(admin)/support',
                params: { initialCount: inbox.unread, cachedData: JSON.stringify(inbox.cachedTickets || []) },
              })
            } />
          </View>
        </View>
      ...
      {/* Floating Boss Chat */}
      <TouchableOpacity style={styles.floatingChat} onPress={() => router.push('/(admin)/boss')} activeOpacity={0.8}>
      ...
      <View style={styles.quickActionIcon}>
        ...
        {notifCount > 0 && (
          <View style={styles.notifBubble}>
            <Text style={styles.notifBubbleText}>{notifCount > 99 ? "99+" : notifCount}</Text>
          </View>
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 24, fontWeight: "900", color: TEXT, marginBottom: 2 },
  headerSubtitle: { fontSize: 14, color: MUTED, fontWeight: "500" },
  ...
  hotBadge: { position: "absolute", top: 2, right: 2, width: 8, height: 8, borderRadius: 4, backgroundColor: DANGER },
  notifBubble: {
    position: "absolute",
    top: -4,
    right: -4,
  ...
  notifBubbleText: { color: "#fff", fontWeight: "bold", fontSize: 11, textAlign: "center" },
  quickActionText: { flex: 1 },
  quickActionTitle: { color: TEXT, fontWeight: "700", fontSize: 15, marginBottom: 2 },
  quickActionSubtitle: { color: MUTED, fontSize: 13 },
  metricsGrid: { flexDirection: "row", gap: 8, marginBottom: 8 },
  metricCard: { flex: 1, backgroundColor: "#f8fafc", borderRadius: 8, padding: 12, borderWidth: 1, borderColor: BORDER, minHeight: 70 },
  metricCardSmall: { minHeight: 60 },
  metricHeader: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 8 },
  metricTitle: { color: MUTED, fontSize: 11, fontWeight: "600", flex: 1 },
