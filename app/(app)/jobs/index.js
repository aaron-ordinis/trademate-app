...
function log(tag, data) {
  try {
    console.log("[jobs.index]", tag, data || {});
  } catch {}
}
...
export default function JobsIndex() {
  ...
  const buzz = () => {
    if (haptic.current && haptic.current.selectionAsync) {
      haptic.current.selectionAsync();
    }
  };
  ...
  const right = (
    <View style={{ flexDirection: "row", gap: 8 }}>
      ...
      <TouchableOpacity
        style={st.iconBtn}
        onPress={function () {
          const nextMode = mode === "list" ? "calendar" : "list";
          setMode(nextMode);
          buzz();
          log("mode.toggle", { mode: nextMode });
        }}
        activeOpacity={0.9}
      >
        {mode === "list" ? (
