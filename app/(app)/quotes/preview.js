...
<style>
  ...
  #strip{
    ...
    justify-content:center; /* center when single page */
  }
  .pageWrap{
  ...
  canvas{
    display:block;
    width:calc(var(--pageW) - 16px); /* snug fit */
...
(function(){
  ...
  async function renderPage(pdf, pageNum, canvas){
    ...
    const cssW = canvas.clientWidth;                 // fit to available width
...
export default function Preview() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const { data } = await supabase.from('profiles').select('branding').eq('id', user.id).maybeSingle();
  ...
  const onEdit = () => {
    if (!quoteId) { Alert.alert('Missing quote', 'Go back to the list and open the quote to edit.'); return; }
    if (!isPremium) { router.push('/(app)/account'); return; }
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
      ...
      <View style={styles.actionBar}>
        ...
        <TouchableOpacity onPress={onEdit} disabled={!!busy} style={[styles.actionBtn, !isPremium && styles.locked, busy && styles.busy]}>
          <Text style={[styles.actionTxt, !isPremium && styles.lockedTxt]}>{isPremium ? 'Edit' : 'Premium'}</Text>
        </TouchableOpacity>
