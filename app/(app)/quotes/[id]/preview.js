import { accountHref, quoteHref } from "../../../../lib/nav";
// app/(app)/quotes/[id]/preview.js
...
async function markStatus(quoteId, status) {
  try {
    if (quoteId) await supabase.from("quotes").update({ status }).eq("id", quoteId);
  } catch {}
}
...
<style>
  ...
  #strip{ display:flex;height:100%;width:100%; overflow-x:auto;overflow-y:hidden;
  ...
  canvas{ display:block; width:calc(var(--pageW) - 16px); height:auto;margin:8px; border-radius:14px;background:#fff;
...
export default function Preview() {
  ...
  const passedPdfUrl = rawUrl ? tryDecode(rawUrl) : "";
  ...
  // premium check (controls Edit button)
  useEffect(() => {
    (async () => {
      try {
        ...
        const { data } = await supabase
          ...
          .eq("id", user.id)
          .maybeSingle();
        setIsPremium(String(data?.branding || "").toLowerCase() === "premium");
      } catch {
        setIsPremium(false);
      }
    })();
  ...
  // If no url provided, ask Supabase for quotes.pdf_url
  ...
  const loadBase64 = useCallback(async () => {
    try {
      ...
      const { status, uri } = await FileSystem.downloadAsync(withBust(url), target);
  ...
  const onSave = async () => {
    try {
      ...
      if (Platform.OS === "android") {
        ...
        if (!perm.granted) {
          Alert.alert("Choose a folder to save the PDF.");
          return;
        }
    ...
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]}>
        ...
        {(loading || !viewerHtml) && (
          <View style={styles.loading}>
            ...
            <Text style={styles.loadingText}>
              {fatal ? fatal : "Preparing your PDF…"}
            </Text>
            {!!fatal && (
      ...
      <View style={styles.actionBar}>
        <TouchableOpacity
          onPress={onShare}
          disabled={!!busy || !!fatal}
          style={[styles.actionBtn, busy === "share" && styles.busy]}
        >
        ...
        <TouchableOpacity
          ...
          style={[
            styles.actionBtn,
            !isPremium && styles.locked,
            busy && styles.busy,
          ]}
        >
          <Text style={[styles.actionTxt, !isPremium && styles.lockedTxt]}>
            {isPremium ? "Edit" : "Premium"}
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={onSave}
          disabled={!!busy || !!fatal}
          style={[styles.actionBtn, busy === "save" && styles.busy]}
        >
          <Text style={styles.actionTxt}>
        ...
        <TouchableOpacity
          ...
          disabled={!!busy || !!fatal}
          style={[styles.actionBtn, busy === "open" && styles.busy]}
        >
          <Text style={styles.actionTxt}>
